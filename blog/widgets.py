...
class SelectDateTimeWidget(Widget):
    ...
    def render(self, name, value, attrs=None):
        ...
        date_output = '<div class="date-select required"><label>Date</label>%s</div>' % mark_safe(u'\n'.join(date_output))
        time_output = '<div class="time-select required"><label>Time</label>%s</div>' % mark_safe(u'\n'.join(time_output))
